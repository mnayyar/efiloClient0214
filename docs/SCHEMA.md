# Prisma Schema Reference — efilo.ai

> Single-tenant: one Organization record per database. No tenant scoping on any model.
...
model Project {
  ...
  status        String       @default("active") // active, planning, closeout, archived
...
model DocumentChunk {
  ...
  pageNumber    Int?
  sectionRef    String?
  metadata      Json?     // {headings[], tableData, figureRefs[], keywords[]}
...
model ChatSession {
  ...
  project    Project?  @relation(fields: [projectId], references: [id])
  title      String?
  messages   Json      // Array of {id, role, content, sources, timestamp, scope, tokens}
...
## Capability Models (RFI, Compliance, Health, Change, Meeting, Closeout, Enterprise)
...
model ComplianceScore {
  ...
  project       Project   @relation(fields: [projectId], references: [id])
  score         Int       // 0-100
  details       Json      // breakdown by clause type
