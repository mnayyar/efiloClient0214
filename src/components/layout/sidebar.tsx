...
export function Sidebar({ collapsed, onToggle, activeProjectId }: SidebarProps) {
  ...
  // Refetch projects on navigation (catches newly created projects)
  useEffect(() => {
    fetchProjects();
  ...
  function handleAgentClick(agentPath: string) {
    if (activeProjectId) {
      router.push(`/projects/${activeProjectId}/${agentPath}`);
    } else {
      // Refetch before showing picker in case new projects were created
      fetchProjects();
  ...
  function handleProjectPicked(projectId: string) {
    if (pickingFor) {
      router.push(`/projects/${projectId}/${pickingFor}`);
      setPickingFor(null);
    }
  }

  const activeAgents = AGENTS.filter((a) => !("roadmap" in a && a.roadmap));
  const roadmapAgents = AGENTS.filter((a) => "roadmap" in a && a.roadmap);
            ...
            {activeAgents.map((agent) => {
              ...
              return (
                ...
                >
                  {/* Left accent bar */}
                  {isActive && (
                    <div className={cn("absolute left-0 top-2 bottom-2 w-[3px] rounded-full", agent.activeBar)} />
                  )}
          ...
          <div className="flex flex-col gap-1">
            {roadmapAgents.map((agent) => (
              ...
              >
                ...
                {!collapsed && (
                  <span className="truncate text-slate-300 transition-colors group-hover:text-slate-200">{agent.label}</span>
                )}
              </div>
            ))}
          </div>
