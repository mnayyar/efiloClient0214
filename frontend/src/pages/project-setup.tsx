  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (contractValue) {
        ...
        if (!isNaN(parsed) && parsed > 0) body.contractValue = parsed;
      }
      ...
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.detail || data.error || "Failed to create project.");
        return;
      }
...
function EditProjectDialog({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.detail || data.error || "Failed to update project.");
        return;
      }
