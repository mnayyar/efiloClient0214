...
const mdComponents = {
  ...
  thead: ({ children, ...props }: ComponentPropsWithoutRef<"thead">) => (
    <thead className="border-b border-border-card bg-brand-off-white" {...props}>
      {children}
    </thead>
  ),
  ...
  tr: ({ children, ...props }: ComponentPropsWithoutRef<"tr">) => (
    <tr className="transition-colors hover:bg-brand-off-white/50" {...props}>
      {children}
    </tr>
  ),
  ...
  code: ({ children, className, ...props }: ComponentPropsWithoutRef<"code">) => {
    ...
    return isInline ? (
      <code
        className="rounded bg-brand-off-white px-1.5 py-0.5 font-mono text-xs text-brand-orange"
        {...props}
      >
        {children}
      </code>
  ...
  pre: ({ children, ...props }: ComponentPropsWithoutRef<"pre">) => (
    <pre
      className="mb-3 overflow-x-auto rounded-lg border border-border-card bg-brand-off-white p-3 font-mono text-xs"
      {...props}
    >
      {children}
    </pre>
                      ...
                      <div className="flex items-center gap-1.5">
                        <div className="h-1.5 w-16 overflow-hidden rounded-full bg-border-card">
                          <div
                            ...
                            style={{
                              width: `${Math.min(msg.confidence * 100, 100)}%`,
                            }}
                          />
                        </div>
